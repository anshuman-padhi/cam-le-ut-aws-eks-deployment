...
!include AWSPuml/Compute/EKSCloud.puml
...
' Custom styling
skinparam rectangle {
    BackgroundColor<<internet>> #E3F2FD
    BorderColor<<internet>> #1976D2
    BackgroundColor<<public>> #FFF3E0
    BorderColor<<public>> #F57C00
    BackgroundColor<<private>> #F3E5F5
    BorderColor<<private>> #7B1FA2
    BackgroundColor<<data>> #FFEBEE
    BorderColor<<data>> #C62828
}
...
' External Users
Users(users, "API Clients", "External Applications")
User(admin, "Administrators", "DevOps Team")

AWSCloudGroup(cloud) {
    VPCGroup(vpc, "VPC - 10.0.0.0/16") {
        
        ' Public Subnet - Internet-Facing LoadBalancers
        PublicSubnetGroup(publicSubnet, "Public Subnets (Multi-AZ)") {
            ElasticLoadBalancingNetworkLoadBalancer(tmLB, "Traffic Manager NLB", "Port 443, Internet-Facing")
            ElasticLoadBalancingNetworkLoadBalancer(cuiLB, "Config UI NLB", "Port 443, Internet-Facing")
        }
        
        ' Private Subnet - EKS Cluster
        PrivateSubnetGroup(privateSubnet, "Private Subnets (Multi-AZ)") {
            
            ' EKS Cluster
            rectangle "EKS Cluster" <<private>> {
                EKS(eks, "CAM LE Cluster", "Kubernetes 1.28+")
                ...
                rectangle "Backend Services" {
                    ...
                    ElasticLoadBalancingNetworkLoadBalancer(paLB, "Platform API NLB", "Port 8080, Internal")
                }
                
                ' Data & Cache Layer
                rectangle "Data & Cache Layer" {
                    component "Apache Ignite Cache" as cache [
                        **Apache Ignite Cache**
                        ----
                    ...
                    component "Loader Cron Jobs" as lcron [
                        **Loader Cron**
                        ----
                        Full Sync: Daily 2 AM
                        Delta Sync: Every 15 min
                        On-Prem Load: Every 15 min
                    ]
                }
            }
            
            ' Database Layer
            rectangle "Database Layer" <<data>> {
                Aurora(aurora, "Aurora MySQL", "Cluster")
                AuroraMySQLInstance(dbPrimary, "Primary Instance", "db.r6g.large\nWriter Endpoint")
                AuroraMySQLInstance(dbReplica, "Replica Instance", "db.r6g.large\nReader Endpoint\nMulti-AZ")
            }
        }
        
        ' Container Registry
        rectangle "Container Registry" <<internet>> {
            ElasticContainerRegistry(ecr, "Amazon ECR", "9 Container Images")
...
tmLB -down-> tm : Routes to Pods
cuiLB -down-> cui : Routes to Pods
paLB ..> pa : Routes to Pods

' Frontend to Backend
cui -right-> paLB : REST API\nManagement\nOperations
tm -down-> paLB : Internal API\nRuntime\nOperations

' Cache Interactions (Dotted for reads)
tm ..> cache : Read Cache\nAPI Definitions
pa ..> cache : Read Cache\nConfigurations
loader ==> cache : Populate Cache\nSync from DB

' Cron Job Triggers
lcron ..> loader : Scheduled\nTriggers
...
' Aurora grouping
aurora -down-> dbPrimary
aurora -down-> dbReplica

' ECR Image Pulls (at deployment time)
tm ..> ecr : Pull Images
pa ..> ecr : Pull Images
cui ..> ecr : Pull Images
cache ..> ecr : Pull Images
loader ..> ecr : Pull Images
ecr -down-> images : Contains

' Legend
legend right
    |= Symbol |= Meaning |
    | <&arrow-thick-right> | Runtime Data Flow |
    | <&arrow-thick-right> (dotted) | Read Operations / Image Pulls |
    | <&arrow-thick-right> (bold) | Cache Population |
...
> **Note**: The PlantUML diagram above uses official AWS architecture icons from the AWS Icons for PlantUML library. When rendered, it will display professional AWS service icons for EKS, Aurora, ECR, Network Load Balancers, and other components.
